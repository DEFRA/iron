...
describe('Iron', () => {
    ...
    describe('unseal()', () => {
        ...
        it('unseals a ticket', async () => {
            ...
            const unsealed = await Iron.unseal(Buffer.from(ticket).toString('hex'), password, Iron.defaults);
            expect(unsealed).to.equal(obj);
        });
        ...
        it('returns an error when number of sealed components is wrong', async () => {
            ...
            const err = await expect(Iron.unseal(Buffer.from(ticket).toString('hex'), password, Iron.defaults)).to.reject('Incorrect number of sealed components');
            expect(err.isBoom).to.be.true();
        });
        ...
        it('returns an error when password is missing', async () => {
            ...
            const err = await expect(Iron.unseal(Buffer.from(ticket).toString('hex'), null, Iron.defaults)).to.reject('Empty password');
            expect(err.isBoom).to.be.true();
        });
        ...
        it('returns an error when mac prefix is wrong', async () => {
            ...
            const err = await expect(Iron.unseal(Buffer.from(ticket).toString('hex'), password, Iron.defaults)).to.reject('Wrong mac prefix');
            expect(err.isBoom).to.be.true();
        });
        ...
        it('returns an error when integrity check fails', async () => {
            ...
            const err = await expect(Iron.unseal(Buffer.from(ticket).toString('hex'), password, Iron.defaults)).to.reject('Bad hmac value');
            expect(err.isBoom).to.be.true();
        });
        ...
        it('returns an error when decryption fails', async () => {
            ...
            const err = await expect(Iron.unseal(Buffer.from(ticket).toString('hex'), password, Iron.defaults)).to.reject('Invalid character');
            expect(err.isBoom).to.be.true();
        });
        ...
        it('returns an error when decrypted object is invalid', async () => {
            ...
            const ticket = macBaseString + '*' + mac.salt + '*' + mac.digest;
            const err = await expect(Iron.unseal(Buffer.from(ticket).toString('hex'), password, Iron.defaults)).to.reject(/Failed parsing sealed object JSON: Unexpected token a/);
            expect(err.isBoom).to.be.true();
        });
        ...
        it('returns an error when expired', async () => {
            ...
            const mac = await Iron.hmacWithPassword(password, options, macBaseString);
            const ticket = macBaseString + '*' + mac.salt + '*' + mac.digest;
            const err = await expect(Iron.unseal(Buffer.from(ticket).toString('hex'), password, Iron.defaults)).to.reject('Expired seal');
            expect(err.isBoom).to.be.true();
        });
        ...
        it('returns an error when expiration NaN', async () => {
            ...
            const mac = await Iron.hmacWithPassword(password, options, macBaseString);
            const ticket = macBaseString + '*' + mac.salt + '*' + mac.digest;
            const err = await expect(Iron.unseal(Buffer.from(ticket).toString('hex'), password, Iron.defaults)).to.reject('Invalid expiration');
            expect(err.isBoom).to.be.true();
        });
    });
});
